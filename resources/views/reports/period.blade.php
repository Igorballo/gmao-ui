...
<head>
    ...
    <style>
        * { font-family: DejaVu Sans, sans-serif; }
        ...
        .stats td { width: 16.6%; background: #f8fafc; border: 1px solid #e2e8f0; padding: 10px; text-align: center; }
...
<body>
    <div class="header">
        ...
        <div class="title">
            Rapport technique consolidé —
            du {{ $debut->locale('fr')->isoFormat('D MMMM YYYY') }}
            au {{ $fin->locale('fr')->isoFormat('D MMMM YYYY') }}
        </div>
        <div class="meta">{{ $stats['nbJours'] }} jour(s) · Généré le {{ $genereLe->format('d/m/Y à H:i') }}</div>
    </div>

    <table class="stats">
        <tr>
            <td><div class="val">{{ $stats['nbJours'] }}</div><div class="lbl">Jours</div></td>
    ...
    <h2>Pannes de la période</h2>
    @if ($pannes->isEmpty())
        <p class="muted">Aucune panne signalée sur la période.</p>
    @else
        <table class="data">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($pannes as $panne)
                    ...
                    <tr>
                        ...
                        <td>{{ $panne->date_panne->format('d/m H:i') }}</td>
    ...
    <h2>Interventions de la période</h2>
    @if ($interventions->isEmpty())
        <p class="muted">Aucune intervention démarrée sur la période.</p>
    @else
        <table class="data">
            <thead>
                <tr>
                    ...
                    <th style="width:18%">Machine</th>
                    <th style="width:33%">Cause / opérations</th>
                    <th style="width:20%">Maintenancier</th>
            ...
            <tbody>
                @foreach ($interventions as $intervention)
                    ...
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $intervention->demarree_le?->format('d/m H:i') ?? '—' }}</td>
